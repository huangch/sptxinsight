...
Reads ``niche-outputs-csv/cells/<id>.csv`` produced by ``sptxinsight niche`` and, for
...
run without the deep-learning ``[niche]`` extra.
"""
...
# sync with ``_NICHE_MODE_SPEC`` in niche_generation.py but defined locally so this
# module stays free of the heavy deep-learning ``[niche]`` extra.
_MODE_SPEC = {
    "celltype": ("niche-outputs-csv", "niche_", ""),
    "expression": ("niche-gex-outputs-csv", "gexniche_", "-gex"),
    "both": ("niche-hybrid-outputs-csv", "hniche_", "-hybrid"),
}


def _niche_columns(columns: List[str], prefix: str = "niche_") -> List[str]:
...
def niche_profile(
    ...
    ``niche-outputs-csv``/``niche_``; ``expression`` -> ``niche-gex-outputs-csv``/
    ``gexniche_``; ``both`` -> ``niche-hybrid-outputs-csv``/``hniche_``).
    ...
    saved to ``<results-dir>/niche-profile-composition<suffix>.csv`` and
    ``<results-dir>/niche-profile-markers<suffix>.csv``.
    """
    ...
    if not csvs:
        raise FileNotFoundError(
            f"No niche cell CSVs found under {cells_dir}. "
            f"Run `sptxinsight niche --niche-mode {mode}` first."
        )

    head = pd.read_csv(csvs[0], nrows=1)
    niche_cols = _niche_columns(list(head.columns), prefix)
    if not niche_cols:
        raise ValueError(
            f"No {prefix} columns in {csvs[0]}; was this produced by "
            f"`sptxinsight niche --niche-mode {mode}`?"
        )
    ...
    if write:
        comp.to_csv(results_dir / f"niche-profile-composition{suffix}.csv")
        if markers is not None:
            markers.to_csv(
                results_dir / f"niche-profile-markers{suffix}.csv", index=False
            )
...
def niche_agreement(
    ...
    Requires both ``niche-outputs-csv/cells/`` (``niche_*``) and
    ``niche-gex-outputs-csv/cells/`` (``gexniche_*``). For each shared sample the two
    ...
    ``<results-dir>/niche-agreement.csv``.
    """
    results_dir = Path(str(results_dir))
    ct_dir = results_dir / "niche-outputs-csv" / "cells"
    gx_dir = results_dir / "niche-gex-outputs-csv" / "cells"
    ...
    for name in common:
        ct = pd.read_csv(ct_csvs[name], usecols=lambda c: c.startswith("niche_"))
        gx = pd.read_csv(gx_csvs[name], usecols=lambda c: c.startswith("gexniche_"))
        ct_cols = _niche_columns(list(ct.columns), "niche_")
        gx_cols = _niche_columns(list(gx.columns), "gexniche_")
    ...
    crosstab.index = [f"niche_{i}" for i in crosstab.index]
    crosstab.columns = [f"gexniche_{j}" for j in crosstab.columns]

    if write:
        crosstab.to_csv(results_dir / "niche-agreement.csv")
