...
- **Why `--runner` (not `-b` / `--backend`):** sptxinsight's CLI itself has a global `--backend` flag that selects the I/O backend (`anndata|zarr|spatialdata`). The wrapper uses `--runner` for the orthogonal "where do I run the CLI" question, keeping the two flag spaces disjoint. (`wsinsight.sh` DOES use `-b` because the wsinsight CLI has no `--backend` flag — divergence is intentional.) If a user types `-b docker` on this wrapper (muscle memory from `wsinsight.sh`), the wrapper prints a hint rather than silently forwarding it to sptxinsight's `--backend` parser.
